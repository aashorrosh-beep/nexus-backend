...
def fetch_live_dropship_feed(room_name: str):
    # Simulates pulling 10 raw items from a global dropship supplier network
    items = []
    for i in range(1, 11):
...
async def vp_marketing(raw_item: dict, room_name: str):
    # Sanitizes raw supplier titles into premium retail branding
    clean_name = raw_item['raw_title'].split('Item')[0].replace('2026 Trending ', 'PREMIUM ').strip()
    item_number = raw_item['raw_title'].split('Item')[1].split()[0]
    
    return {
        "storefront": room_name.upper(),
        "name": f"🔥 {clean_name.upper()} - ASSET {item_number}",
        "cost": raw_item['wholesale_cost'],
        "base_img": raw_item['base_img'],
        "marketing_copy": f"Verified global dropship asset. Professionally curated for the {room_name} collection with guaranteed high-velocity demand."
    }

async def vp_logistics(item: dict):
    item['shippingText'] = f"SECURE DROPSHIP NETWORK: {random.randint(4, 7)} DAYS"
    return item

async def vp_media_security(item: dict):
    item['images'] = [item['base_img']]
    item['specs'] = ProductSpecs(
        manufacturer="NEXUS Global Verified Partner", 
